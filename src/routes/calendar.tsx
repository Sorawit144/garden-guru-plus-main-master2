...
export const Route = createFileRoute("/calendar")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "ตารางงานใส่ปุ๋ย รดน้ำ ฉีดยา และเก็บเกี่ยว พร้อมการแจ้งเตือน",
      },
...
const TYPE_ICON: Record<TaskType, typeof Droplets> = {
  รดน้ำ: Droplets,
  ใส่ปุ๋ย: Sprout,
  ฉีดยา: BadgeCheck,
  เก็บเกี่ยว: PackageCheck,
  ตัดแต่ง: Scissors,
  อื่นๆ: ClipboardList,
};
...
function CalendarPage() {
  ...
  const [smartTaskApprovalMode, setSmartTaskApprovalMode] = useState<
    "team_lead" | "farm_manager" | "qa"
  >("team_lead");
  ...
  const [assignmentApprovalMode, setAssignmentApprovalMode] = useState<
    "team_lead" | "farm_manager" | "qa"
  >("team_lead");
  ...
  if (dragonfly.isDemoMode) {
    ...
    const farmSiteIds = Array.from(
      new Set(
        dragonfly.state.tasks
          .filter((task) => (task.farmId ?? "FARM-PRIMARY") === demoFarmFilter)
    ...
    const demoSiteOptions = [
      ...
      ...Array.from(
        new Map([
          ...dragonfly.state.sites
            .filter((site) => (site.farmId ?? "FARM-PRIMARY") === demoFarmFilter)
            .map(
              (site) =>
                [site.id, { value: site.id, label: `${site.code} · ${site.name}` }] as const,
            ),
          ...farmSiteIds.map(
            (siteId) => [siteId, { value: siteId, label: getDemoSiteLabel(siteId) }] as const,
          ),
        ]).values(),
      ),
    ];
    const scopedTaskPlots = dragonfly.state.plots.filter((plot) => {
      ...
      const matchesSite =
        demoSiteFilter === "ทั้งหมด" ||
        plot.siteId === demoSiteFilter ||
    ...
    const tasksInScope = dragonfly.state.tasks.filter((task) => {
      const plot = dragonfly.state.plots.find(
        (item) => item.id === task.plot || item.name === task.plot,
      );
      ...
      const matchesKnownPlot =
        scopedPlotIds.size === 0 ||
        scopedPlotIds.has(task.plot) ||
        scopedTaskPlots.some((item) => item.name === task.plot);
      const isTeamTask = task.origin === "team" || Boolean(task.team);
      const matchesWorkspace =
        dragonfly.workspaceContext === "personal"
          ? !isTeamTask
          : isTeamTask || task.origin === "system";
    ...
    const demoPlotOptions = [
      "ทั้งหมด",
      ...Array.from(new Set(tasksInScope.map((task) => task.plot))).map((plotId) => {
        const plot = dragonfly.state.plots.find(
          (item) => item.id === plotId || item.name === plotId,
        );
        return { value: plotId, label: plot ? `${plotId} · ${plot.name} · ${plot.crop}` : plotId };
      }),
    ];
    const getEffectiveTaskStatus = (task: (typeof tasksInScope)[number]) =>
      task.status === "Planned"
        ? task.team || task.assignedWorkerId
          ? "Assigned"
          : "Unassigned"
        : task.status;
    const demoStatusOptions = [
      "ทั้งหมด",
      ...Array.from(new Set(tasksInScope.map(getEffectiveTaskStatus))),
    ];
    const demoTypeOptions = [
      "ทั้งหมด",
      ...Array.from(new Set(tasksInScope.map((task) => task.type))),
    ];
    const summarySmartTasks = tasksInScope.filter(
      (task) =>
        (demoPlotFilter === "ทั้งหมด" || task.plot === demoPlotFilter) &&
        (demoTypeFilter === "ทั้งหมด" || task.type === demoTypeFilter) &&
        (demoOriginFilter === "ทั้งหมด" ||
          (demoOriginFilter === "team"
            ? task.origin === "team" || Boolean(task.team)
            : task.origin !== "team" && !task.team)) &&
        isTaskInPeriod(task.scheduledFor, demoTimeFilter, demoCustomRange),
    );
    const filteredSmartTasks = summarySmartTasks.filter(
      (task) => demoStatusFilter === "ทั้งหมด" || getEffectiveTaskStatus(task) === demoStatusFilter,
    );
    const canCreatePersonalTask = dragonfly.workspaceContext === "personal";
    const canCreateTeamTask =
      dragonfly.workspaceContext === "organization" &&
      dragonfly.effectiveSubscription === "Farm Pro" &&
      !organizationEmployee;
    const crewOptions = Array.from(
      new Set(dragonfly.state.workers.map((worker) => worker.crew)),
    ).map((crew) => ({ value: crew, label: crew }));
    const originLabel = (task: (typeof filteredSmartTasks)[number]) =>
      task.origin === "team" || task.team
        ? "งานทีม"
        : task.origin === "system"
          ? "งานจากระบบ"
    ...
    return (
      <AppShell
        title="ตารางงานสวน"
        subtitle={`${dragonfly.workspaceLabel} · ${dragonfly.effectiveRole} · ติดตามงานตามแปลง ประเภท และสถานะ`}
      >
        ...
        <Card className="space-y-3">
          <SearchableSelect
            ...
            value={demoFarmFilter}
            onChange={(value) => {
              dragonfly.setActiveDashboardFarm(value);
              setDemoFarmFilter(value);
              setDemoSiteFilter("ทั้งหมด");
              setDemoPlotFilter("ทั้งหมด");
            }}
          ...
          <SearchableSelect
            label="โซน"
            options={demoSiteOptions}
            value={demoSiteFilter}
            onChange={(value) => {
              setDemoSiteFilter(value);
              setDemoPlotFilter("ทั้งหมด");
            }}
          ...
          />
          <TimeRangeFilter
            value={demoTimeFilter}
            onChange={setDemoTimeFilter}
            options={[
            ...
            dateRange={demoCustomRange}
            onDateRangeChange={setDemoCustomRange}
          />
          <SearchableSelect
            label="แปลง"
            options={demoPlotOptions}
            value={demoPlotFilter}
            onChange={setDemoPlotFilter}
            allLabel="ทุกแปลง"
            searchPlaceholder="ค้นหารหัส ชื่อแปลง หรือพืช"
          />
          <DropdownFilter
            label="สถานะ"
            values={demoStatusOptions}
            value={demoStatusFilter}
            onChange={setDemoStatusFilter}
            allLabel="ทุกสถานะ"
            getLabel={getCalendarStatusLabel}
          />
          <DropdownFilter
            label="ประเภทงาน"
            values={demoTypeOptions}
            value={demoTypeFilter}
            onChange={setDemoTypeFilter}
            allLabel="ทุกประเภทงาน"
          />
          <DropdownFilter
            label="ประเภทเจ้าของงาน"
            values={dragonfly.workspaceContext === "personal" ? ["ส่วนตัว"] : ["ทีม"]}
            value={dragonfly.workspaceContext === "personal" ? "ส่วนตัว" : "ทีม"}
            onChange={() =>
              setDemoOriginFilter(dragonfly.workspaceContext === "personal" ? "personal" : "team")
            }
            allLabel="ทุกงาน"
          />
          <p className="text-xs text-muted-foreground">
            กำลังแสดง {filteredSmartTasks.length} จาก {tasksInScope.length} งานในฟาร์ม/โซนที่เลือก
          </p>
        </Card>

        {canCreatePersonalTask || canCreateTeamTask ? (
          <button
            data-tour="calendar-create-task"
            onClick={() => {
              setSmartTaskPlot(scopedTaskPlots[0]?.id ?? "");
              setSmartTaskOrigin(canCreateTeamTask ? "team" : "personal");
              setSmartTaskAssigneeMode("crew");
              setSmartTaskTeam(crewOptions[0]?.value ?? "");
              setSmartTaskWorkerId("");
              setSmartTaskDate(getLocalDateKey());
              setShowSmartTaskForm((value) => !value);
            }}
          ...
          <Card className="border-primary/25 bg-primary-soft/35 text-xs text-muted-foreground">
            ปฏิทินนี้แสดงแผนงานของสวน พนักงานสร้างหรือแก้ไขงานไม่ได้ และรับงานผ่านหน้า “งานของฉัน”
            เท่านั้น
          </Card>
        )}

        {showSmartTaskForm ? (
          <Card className="space-y-3 border-primary/30 bg-primary-soft/35">
            <div>
              <p className="text-sm font-semibold text-primary">สร้างงานในปฏิทิน</p>
              <p className="mt-1 text-xs text-muted-foreground">
                ทุกงานจะเป็น Task กลาง และจะเข้า Care Log ของแปลงเมื่อปิดงานแล้ว
              </p>
            </div>
            <input
              value={smartTaskTitle}
              onChange={(event) => setSmartTaskTitle(event.target.value)}
              placeholder="ชื่องาน เช่น ตรวจใบอ่อน"
              className="w-full rounded-lg border border-border bg-card px-3 py-2 text-sm outline-none focus:border-primary"
            />
            <SearchableSelect
              label="แปลง"
              options={scopedTaskPlots.map((plot) => ({
                value: plot.id,
                label: `${plot.id} · ${plot.name} · ${plot.crop}`,
              }))}
              value={smartTaskPlot}
              onChange={setSmartTaskPlot}
              searchPlaceholder="ค้นหารหัส ชื่อแปลง หรือพืช"
            />
            <div className="grid grid-cols-2 gap-2">
              <select
                ...
                onChange={(event) => {
                  ...
                  setSmartTaskType(type);
                  if (type === "Harvest") setSmartTaskApprovalMode("qa");
                }}
                className="rounded-lg border border-border bg-card px-2 py-2 text-xs"
              >
                <option value="Inspection">ตรวจแปลง</option>
                <option value="Irrigation">ให้น้ำ</option>
                <option value="Fertilizer">ใส่ปุ๋ย</option>
                <option value="Pruning">ตัดแต่ง</option>
              ...
              <input
                type="date"
                value={smartTaskDate}
                onChange={(event) => setSmartTaskDate(event.target.value)}
                className="rounded-lg border border-border bg-card px-2 py-2 text-xs"
              />
            </div>
            <div className="grid grid-cols-2 gap-2">
              {(["personal", "team"] as const)
                .filter((origin) => (origin === "team" ? canCreateTeamTask : canCreatePersonalTask))
                .map((origin) => (
                  <button
                    key={origin}
                    onClick={() => setSmartTaskOrigin(origin)}
                    className={`rounded-lg border py-2 text-xs font-semibold ${smartTaskOrigin === origin ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card"}`}
                  >
                    {origin === "personal" ? "งานส่วนตัว" : "งานทีม"}
                  </button>
                ))}
            </div>
              ...
              <>
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className={`rounded-lg border py-2 text-xs font-semibold ${smartTaskAssigneeMode === "crew" ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card"}`}
                  >
                    มอบหมายทั้งทีม
                  </button>
                  <button
                    type="button"
                    onClick={() => setSmartTaskAssigneeMode("person")}
                    className={`rounded-lg border py-2 text-xs font-semibold ${smartTaskAssigneeMode === "person" ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card"}`}
                  >
                    มอบหมายรายบุคคล
                  </button>
                </div>
                {smartTaskAssigneeMode === "crew" ? (
                  <SearchableSelect
                    label="ทีมรับผิดชอบ"
                    options={crewOptions}
                    value={smartTaskTeam}
                    onChange={setSmartTaskTeam}
                    searchPlaceholder="ค้นหาชื่อทีม"
                  />
                ) : (
                  <SearchableSelect
                    label="พนักงานรับผิดชอบ"
                    options={dragonfly.state.workers.map((worker) => ({
                      value: worker.id,
                      label: `${worker.name} · ${worker.crew}`,
                    }))}
                    value={smartTaskWorkerId}
                    onChange={(value) => {
                      setSmartTaskWorkerId(value);
                      setSmartTaskTeam(
                        dragonfly.state.workers.find((worker) => worker.id === value)?.crew ?? "",
                      );
                    }}
                    searchPlaceholder="ค้นหาชื่อพนักงานหรือทีม"
                  />
                )}
                <p className="text-[11px] text-muted-foreground">
                  {smartTaskAssigneeMode === "crew"
                    ? "สมาชิกทุกคนในทีมจะเห็นงานนี้ใน “งานของฉัน” จนกว่าจะมีคนรับงาน"
                    : "เฉพาะพนักงานที่เลือกจะเห็นงานนี้ใน “งานของฉัน”"}
                </p>
                <label className="block text-xs font-semibold text-muted-foreground">
                  ผู้มีสิทธิ์ตรวจรับ
                  <select
                    value={smartTaskApprovalMode}
                    onChange={(event) =>
                      setSmartTaskApprovalMode(event.target.value as typeof smartTaskApprovalMode)
                    }
                    className="mt-1.5 block w-full rounded-lg border border-border bg-card px-3 py-2.5 text-xs text-foreground"
                  >
                    <option value="team_lead">หัวหน้าทีม · ผู้จัดการอนุมัติสำรอง</option>
                    <option value="farm_manager">ผู้จัดการฟาร์มเท่านั้น</option>
                    <option value="qa">เจ้าหน้าที่ QA</option>
                  </select>
                </label>
              </>
            ) : null}
            <button
              onClick={() => {
                const plot = scopedTaskPlots.find((item) => item.id === smartTaskPlot);
                const worker = dragonfly.state.workers.find(
                  (item) => item.id === smartTaskWorkerId,
                );
                const assignedTeam =
                  smartTaskOrigin === "team"
                    ? smartTaskAssigneeMode === "crew"
                      ? smartTaskTeam
                      : (worker?.crew ?? smartTaskTeam)
                    : undefined;
                if (!smartTaskTitle.trim() || !plot) {
                  toast.error("กรอกชื่องานและเลือกแปลงก่อน");
                  return;
                }
                if (smartTaskOrigin === "team" && !assignedTeam) {
                  toast.error("เลือกทีมหรือพนักงานที่รับผิดชอบก่อน");
                  return;
                }
                dragonfly.addTask({
                  title: smartTaskTitle.trim(),
                  plot: plot.id,
                  farmId: plot.farmId ?? demoFarmFilter,
                  siteId: plot.siteId,
                  type: smartTaskType,
                  priority: "Normal",
                  approvalMode: smartTaskOrigin === "personal" ? "self" : smartTaskApprovalMode,
                  origin: smartTaskOrigin,
                  team: assignedTeam,
                  assignedWorkerId:
                    smartTaskOrigin === "team" && smartTaskAssigneeMode === "person"
                      ? smartTaskWorkerId || undefined
                      : undefined,
                  createdBy: dragonfly.effectiveRole,
                  ownerPersonaId: smartTaskOrigin === "personal" ? dragonfly.persona.id : undefined,
                  status: smartTaskOrigin === "team" ? "Assigned" : "Planned",
                  scheduledFor: smartTaskDate,
                });
                setSmartTaskTitle("");
                setSmartTaskApprovalMode("team_lead");
                setShowSmartTaskForm(false);
                toast.success(
                  smartTaskOrigin === "team"
                    ? `มอบหมายงานให้${smartTaskAssigneeMode === "crew" ? `ทีม ${assignedTeam}` : (worker?.name ?? "พนักงาน")}แล้ว`
                    : "สร้างงานส่วนตัวแล้ว",
                );
              }}
        ...
        <div className="space-y-3">
          {filteredSmartTasks.map((task) => {
            const taskPlot = dragonfly.state.plots.find(
              (plot) => plot.id === task.plot || plot.name === task.plot,
            );
            const taskFarmId = task.farmId ?? taskPlot?.farmId ?? "FARM-PRIMARY";
            const assignableWorkers = dragonfly.state.workers.filter(
              (worker) => !worker.farmId || worker.farmId === taskFarmId,
            );
            const assignmentCrewOptions = Array.from(
              new Set(assignableWorkers.map((worker) => worker.crew)),
            ).map((crew) => ({ value: crew, label: crew }));
            const crewKeyword =
              task.type === "Irrigation"
                ...
                : task.type === "Harvest"
                  ...
                  : task.type === "Inspection"
                    ? "protection"
                    : "general";
            const suggestedAssignmentTeam =
              assignableWorkers.find((worker) => worker.plot === task.plot)?.crew ??
              assignmentCrewOptions.find((crew) =>
                crew.value.toLocaleLowerCase().includes(crewKeyword),
              )?.value ??
              assignmentCrewOptions[0]?.value ??
              "";
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {task.plot} · {task.type} · {originLabel(task)}
                          {task.scheduledFor
                        ...
                        <p className="mt-1 text-[11px] text-muted-foreground">
                          ...
                          {task.assignedWorkerId
                            ? ` · ผู้รับผิดชอบ ${dragonfly.state.workers.find((worker) => worker.id === task.assignedWorkerId)?.name ?? task.assignedWorkerId}`
                            : task.team
                              ? ` · มอบหมายทั้งทีม ${task.team}`
                              : " · ยังไม่มอบหมาย"}
                          {task.priority
                            ? ` · ความสำคัญ ${task.priority === "Urgent" ? "เร่งด่วน" : task.priority === "High" ? "สูง" : task.priority === "Low" ? "ต่ำ" : "ปกติ"}`
                            : ""}
                        </p>
                        {task.origin === "team" || task.team ? (
                          <p className="mt-1 text-[11px] text-muted-foreground">
                            ผู้ตรวจรับ: {getTaskReviewerLabel(task)}
                          </p>
                        ) : null}
                      </div>
                      <Badge tone={statusTone(effectiveStatus)}>
                        {getCalendarStatusLabel(effectiveStatus)}
                      </Badge>
                    </div>
                    {task.reason ? (
                    ...
                    {!organizationEmployee && isUnassigned ? (
                      <>
                        ...
                        {assigningTaskId === task.id ? (
                          <div className="mt-3 space-y-3 rounded-lg border border-primary/25 bg-primary-soft/35 p-3">
                            <div>
                              <p className="text-xs font-semibold text-primary">
                                เลือกผู้รับผิดชอบ
                              </p>
                              <p className="mt-1 text-[11px] text-muted-foreground">
                                งานยังไม่เข้าหน้างานของพนักงานจนกว่าจะยืนยันการมอบหมาย
                              </p>
                            </div>
                            <div className="grid grid-cols-2 gap-2">
                              <button
                                ...
                                className={`rounded-lg border py-2 text-xs font-semibold ${assignmentMode === "crew" ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card"}`}
                              >
                                มอบหมายทั้งทีม
                              </button>
                              <button
                                ...
                                className={`rounded-lg border py-2 text-xs font-semibold ${assignmentMode === "person" ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card"}`}
                              >
                                มอบหมายรายบุคคล
                              </button>
                            </div>
                            {assignmentMode === "crew" ? (
                              <SearchableSelect
                                label="ทีมรับผิดชอบ"
                                options={assignmentCrewOptions}
                                value={assignmentTeam}
                                onChange={setAssignmentTeam}
                                searchPlaceholder="ค้นหาชื่อทีม"
                              />
                            ) : (
                              <SearchableSelect
                                ...
                                options={assignableWorkers
                                  .filter(
                                    (worker) =>
                                      !["On Leave", "Unavailable"].includes(worker.status),
                                  )
                                  .map((worker) => ({
                                    value: worker.id,
                                    label: `${worker.name} · ${worker.crew}`,
                                ...
                                onChange={(value) => {
                                  setAssignmentWorkerId(value);
                                  setAssignmentTeam(
                                    assignableWorkers.find((worker) => worker.id === value)?.crew ??
                                      "",
                                  );
                                }}
                                searchPlaceholder="ค้นหาชื่อพนักงานหรือทีม"
                            ...
                            <label className="block text-xs font-semibold text-muted-foreground">
                              ผู้มีสิทธิ์ตรวจรับ
                              <select
                                value={assignmentApprovalMode}
                                onChange={(event) =>
                                  setAssignmentApprovalMode(
                                    event.target.value as typeof assignmentApprovalMode,
                                  )
                                }
                                className="mt-1.5 block min-h-11 w-full rounded-lg border border-border bg-card px-3 text-xs text-foreground"
                              >
                                <option value="team_lead">
                                  หัวหน้าทีม · ผู้จัดการอนุมัติสำรอง
                                </option>
                                <option value="farm_manager">ผู้จัดการฟาร์มเท่านั้น</option>
                                <option value="qa">เจ้าหน้าที่ QA</option>
                              </select>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              ...
                              <button
                                type="button"
                                onClick={() => {
                                  const worker = assignableWorkers.find(
                                    (item) => item.id === assignmentWorkerId,
                                  );
                                  const team =
                                    assignmentMode === "person"
                                      ? (worker?.crew ?? assignmentTeam)
                                      : assignmentTeam;
                                  if (!team || (assignmentMode === "person" && !worker)) {
                                    toast.error(
                                      assignmentMode === "person"
                                        ? "เลือกพนักงานก่อนมอบหมาย"
                                        : "เลือกทีมก่อนมอบหมาย",
                                    );
                                    return;
                                  }
                                  const result = dragonfly.assignTask(task.id, {
                                    team,
                                    assignedWorkerId:
                                      assignmentMode === "person" ? worker?.id : undefined,
                                    approvalMode: assignmentApprovalMode,
                                  });
                                  if (!result.ok) {
                                  ...
                                  toast.success(
                                    assignmentMode === "person"
                                      ? `มอบหมายให้ ${worker?.name} แล้ว`
                                      : `มอบหมายให้ทีม ${team} แล้ว`,
                                  );
                                }}
                      ...
                      <>
                        <div className="mt-3 grid grid-cols-3 gap-2">
                          ...
                          <button
                            onClick={() => {
                              const reason =
                                window.prompt("เหตุผลที่เลื่อน/ข้ามงาน", "Rain") ?? "Other";
                        ...
                        <button
                          onClick={() => {
                            const reason =
                              window.prompt("เหตุผลที่ข้ามงาน", "Worker unavailable") ?? "Other";
                      ...
                      <p className="mt-3 rounded-lg bg-muted px-3 py-2 text-xs text-muted-foreground">
                        ในบริบทองค์กร พนักงานอัปเดตได้เฉพาะงานที่มอบหมายผ่านหน้า “งานของฉัน”
                      </p>
                    ) : null}
                  </div>
                </div>
    ...
    <AppShell title="ปฏิทินงาน" subtitle="วางแผนงานเกษตรรายวัน">
      ...
      {tasksForDate.length === 0 ? (
        <Card className="text-center py-6">
          <ClipboardList className="mx-auto mb-2 size-7 text-primary" />
          <p className="text-sm text-muted-foreground">ยังไม่มีงานในวันนี้</p>
          <p className="text-xs text-muted-foreground mt-1">
            กดปุ่ม "เพิ่มงานใหม่" เพื่อวางแผนได้เลย
          </p>
        </Card>
      ) : (
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {plot && (
                    <p className="flex items-center gap-1 text-xs text-muted-foreground">
                      <FarmIcon crop={plot.crop} className="size-3.5" />
                      {plot.name}
                    </p>
                  )}
                </div>
  ...
  return (
    <label className="block text-xs font-medium text-muted-foreground">
      ...
      <select
        ...
        className="mt-1.5 block min-h-11 w-full rounded-lg border border-border bg-card px-3 text-sm font-semibold text-foreground outline-none focus:border-primary"
      >
        {values.map((item) => (
          <option key={item} value={item}>
            {item === "ทั้งหมด" ? allLabel : (getLabel?.(item) ?? item)}
          </option>
        ))}
      </select>
    </label>
...
function getDemoSiteLabel(siteId: string) {
  return (
    (
      {
        "NORTH-A": "NORTH-A · โซนเนินเหนือ",
        "NORTH-B": "NORTH-B · โซนเชิงเขา",
        "EAST-A": "EAST-A · โซนริมคลอง",
